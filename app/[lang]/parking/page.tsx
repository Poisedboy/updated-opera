...
const ParkingPage = async ({
	params: { lang },
}: {
	params: { lang: Locale };
}) => {
	const { ParkingPage } = await getDictionary(lang);
	const title = ParkingPage.title;
	const mapsLinks = ParkingPage.addresses;
	return <Parking title={title} mapsLinks={mapsLinks} />;
};
